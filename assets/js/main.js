$(document).ready(function () {
  ...
  function displayGoals(goals) {
    ...
    if (goals.length === 0) {
      container.append(
        '<p class="text-muted">No goals found. Click "New Goal" to create one.</p>'
      );
      return;
    }
                    ...
                    <div class="card h-100">
                        <div class="card-body">
                            ...
                            <div class="mt-3">
                                <span class="badge bg-${getStatusColor(
                                  goal.status
                                )}">${formatStatus(goal.status)}</span>
                            </div>
                            <div class="mt-3">
                                <small class="text-muted">Deadline: ${formatDate(
                                  goal.deadline
                                )}</small>
                            </div>
                        </div>
                        <div class="card-footer bg-transparent">
                            <div class="btn-group w-100" role="group">
                                <a href="goal-details.php?id=${
                                  goal.id
                                }" class="btn btn-outline-primary btn-sm">
                                    <i class="bi bi-list-check"></i> Details
                                </a>
                                <button class="btn btn-outline-secondary btn-sm edit-goal" data-goal-id="${
                                  goal.id
                                }">
                                    <i class="bi bi-pencil"></i> Edit
                                </button>
                                <button class="btn btn-outline-danger btn-sm delete-goal" data-goal-id="${
                                  goal.id
                                }">
    ...
    milestones.forEach((milestone) => {
      const item = $(`
                <div class="milestone-item d-flex justify-content-between align-items-center mb-2">
                    ...
                    <button class="btn btn-sm btn-danger delete-milestone" data-milestone-id="${
                      milestone.id
                    }">
  ...
  function updateDashboard(data) {
    ...
    const todoCompletion =
      data.milestones.total_todos > 0
        ? Math.round(
            (data.milestones.completed_todos / data.milestones.total_todos) *
              100
          )
        : 0;
  ...
  function updateGoalPriorityChart(data) {
    ...
    const priorities = data.map(
      (item) => item.priority.charAt(0).toUpperCase() + item.priority.slice(1)
    );
    ...
    milestones.forEach((milestone) => {
      ...
      const progress =
        milestone.total_todos > 0
          ? Math.round(
              (milestone.completed_todos / milestone.total_todos) * 100
            )
          : 0;
      ...
      const item = $(`
                <div class="list-group-item">
                    ...
                    <p class="mb-1 small text-muted">Goal: ${
                    ...
                    <small class="text-muted">
                        ${milestone.completed_todos}/${
        milestone.total_todos
      } todos completed
                    </small>
                </div>
            `);
